...
async def analyze_video_frames(
    ...
    interval_seconds: int = Form(
        5, description="Time interval in seconds for frame analysis"
    ),
    use_fallback: bool = Form(
        False, description="Use fallback analysis if search API fails"
    ),
):
    """
    ...
    if interval_seconds < 1 or interval_seconds > 60:
        raise HTTPException(
            status_code=400, detail="Interval must be between 1 and 60 seconds"
        )
    ...
    with tempfile.NamedTemporaryFile(
        ...
        try:
            ...
            if use_fallback:
                frame_analysis = client.get_frame_by_frame_analysis_fallback(
                    video_id, interval_seconds
                )
            else:
                frame_analysis = client.get_frame_by_frame_analysis(
                    video_id, interval_seconds
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Frame analysis failed: {str(e)}"
            )
        finally:
            # Clean up temporary file
            try:
