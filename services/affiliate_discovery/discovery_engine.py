...
class AffiliateDiscoveryEngine:
    ...
    async def discover_affiliate_programs(
        self, request: SearchRequest
    ) -> ProductSearchResult:
        """Main discovery method that orchestrates the entire process"""
        logger.info(
            f"Starting affiliate program discovery for keywords: {request.keywords}"
        )

        # Check cache first
        cached_result = simple_cache.get(
            "affiliate_discovery", keywords=request.keywords
        )
        if cached_result:
        ...
        try:
            ...
            search_result = await self.groq_client.search_affiliate_programs(
                request.keywords
            )
            ...
            if search_result["success"]:
                if "programs" in search_result:
                    ...
                    programs = self._parse_programs(
                        {"programs": search_result["programs"]}, request
                    )
                else:
                    ...
                    if extraction_result["success"]:
                        programs = self._parse_programs(
                            extraction_result["data"], request
                        )
            ...
            return self._create_empty_result(
                request.keywords, f"Discovery error: {str(e)}"
            )

    def _parse_programs(
        self, extracted_data: Dict[str, Any], request: SearchRequest
        ...
        for program_data in extracted_data.get("programs", []):
            try:
                ...
                logger.warning(
                    f"Failed to parse program data: {program_data}, error: {e}"
                )
                continue

        return programs

    def _filter_and_limit_programs(
        self, programs: List[AffiliateProgram], request: SearchRequest
    ) -> List[AffiliateProgram]:
        ...
        for program in programs:
            ...
            if (
                (
                    program.program_type == "marketplace"
                    and not request.include_marketplaces
                )
                or (
                    program.program_type == "direct"
                    and not request.include_direct_programs
                )
                or (program.program_type == "network" and not request.include_networks)
            ):
                continue
    ...
    def _guess_category(self, keywords: List[str]) -> str:
        ...
        if any(
            word in keywords_lower
            for word in ["clothing", "shirt", "dress", "pants", "fashion", "uniqlo"]
        ):
            return "Fashion & Clothing"
        elif any(
            word in keywords_lower
            for word in ["tech", "electronics", "gadget", "phone", "computer"]
        ):
            return "Technology"
        elif any(
            word in keywords_lower
            for word in ["fitness", "protein", "supplement", "gym"]
        ):
            return "Health & Fitness"
        elif any(
            word in keywords_lower
            for word in ["gaming", "mouse", "keyboard", "headset"]
        ):
            return "Gaming"
        else:
            return "General"
    ...
    def _create_empty_result(
        self, keywords: List[str], reason: str
    ) -> ProductSearchResult:
