...
class ContentAnalyzer:
    ...
    def analyze_content_style(self, channel: ChannelInfo) -> Dict:
        ...
        category_scores = self._calculate_weighted_scores(titles, all_tags, channel.description)
        ...
        return {
            ...
            "creator_insights": self._generate_creator_insights(channel, upload_analysis, performance_analysis),
            "monetization_indicators": self._assess_monetization_readiness(channel, primary_category, performance_analysis["engagement_rate"])
        }
    ...
    def _get_empty_analysis(self) -> Dict:
        ...
        return {
            ...
            "video_style": "unknown",
            "performance_metrics": {},
            "recent_content": {"video_titles": [], "upload_dates": [], "title_themes": [], "common_tags": []},
            "creator_insights": {},
    ...
    def _calculate_weighted_scores(self, titles: List[str], all_tags: List[str], description: Optional[str]) -> Dict[str, int]:
        ...
        for category, keywords in self.content_categories.items():
            ...
            score += sum(title_text.count(keyword) * title_weight for keyword in keywords)
    ...
    def _analyze_upload_patterns(self, videos: List[VideoInfo]) -> Dict:
        ...
        intervals = [(dates[i-1] - dates[i]).days for i in range(1, min(len(dates), 6))]
    ...
    def _get_common_tags(self, all_tags: List[str], limit: int) -> List[str]:
        ...
        common_tags = sorted(tag_counts.items(), key=lambda x: x[1], reverse=True)[:limit]
        return [tag for tag, count in common_tags]

    def _generate_creator_insights(self, channel: ChannelInfo, upload_analysis: Dict, performance_analysis: Dict) -> Dict:
        """Generate insights about the creator"""
        videos = channel.videos
        engagement_rate = performance_analysis.get("engagement_rate", 0)
        
        return {
            "uses_tags_effectively": len(self._extract_all_tags(videos)) > len(videos) * 3,
            "title_consistency": self._check_title_consistency([v.title for v in videos]),
            ...
            "upload_consistency": upload_analysis["style"] in ["daily", "weekly", "bi-weekly"],
    ...
    def _assess_monetization_readiness(self, channel: ChannelInfo, primary_category: str, engagement_rate: float) -> Dict:
        ...
        return {
            ...
            "course_creation_potential": primary_category in ["education_tutorial", "tech_programming", "business_finance"] 
        }
    ...
    def _calculate_niche_specificity(self, category_scores: Dict[str, int], primary_category: str) -> Dict:
    ...
    def _detect_cross_niche_patterns(self, category_scores: Dict[str, int]) -> List[str]:
    ...
    def _assess_niche_monetization_potential(self, primary_category: str, specificity: float) -> Dict:
        ...
        return {
            "niche_value": "high" if primary_category in high_value_niches else "standard",
