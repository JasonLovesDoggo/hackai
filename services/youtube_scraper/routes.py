...
async def scrape_channel(request: ScrapeRequest):
    try:
        channel_info = await scraper.get_channel_info(
            request.channel_id, request.max_videos
        )
...
async def get_channel_info(channel_input: str, max_videos: int = 20):
    try:
        ...
        if not channel_info:
            raise HTTPException(
                status_code=404, detail=f"Channel {channel_input} not found"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching channel info: {str(e)}"
        )
